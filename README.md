...
MESSAGES = [
    ...
    "Stop ignoring me.",
    "Your microphone is now active. Just kidding. Or am I?"
]
...
def speak(text):
    engine = pyttsx3.init()
    engine.setProperty('rate', 150)  # slower = creepier
    engine.say(text)
    engine.runAndWait()

def show_message(text):
    title = random.choice(WINDOW_TITLES)
    win32gui.MessageBox(0, text, title, win32con.MB_OK | win32con.MB_ICONINFORMATION)

def main():
    print("Creepy voice assistant activated.")
    for _ in range(7):
        msg = random.choice(MESSAGES)
        Thread(target=speak, args=(msg,), daemon=True).start()
        show_message(msg)
        time.sleep(random.uniform(3, 6))

    final_message = "You’ve passed the evaluation."
    Thread(target=speak, args=(final_message,), daemon=True).start()
...
Run it:
```bash
python voice_creep.py
```
Creating a Clickable Desktop Icon
#### #1 Convert Script to EXE

- Navigate to this link: https://www.iconarchive.com/
- Search your desired icon. For example: voice
- Click All Downloads Format
- Choose Windows: Download ICO
- Rename the file into voice
- Move the voice.ico to your folder
